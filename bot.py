...
def init_db() -> None:
    with db_connect() as conn:
        ...
        """)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_hist_user ON application_history(user_id, created_at DESC)")
        conn.execute("CREATE INDEX IF NOT EXISTS idx_hist_date ON application_history(event_date, created_at DESC)")
        conn.execute("""
        ...
        migrated = conn.execute("SELECT value FROM settings WHERE key='history_migrated_v3'").fetchone()
        if not migrated:
            for row in conn.execute("SELECT * FROM applications").fetchall():
                created_at = row["created_at"] or now_kst()
                event_date = created_at[:10] if len(created_at) >= 10 else today_kst()
                conn.execute("""
                    INSERT INTO application_history
                    (user_id,name,username,status,event_date,created_at,admin_notified)
                    VALUES (?,?,?,?,?,?,1)
                """, (row["user_id"],row["name"],row["username"],row["status"] or "pending",event_date,created_at))
            conn.execute("INSERT OR REPLACE INTO settings(key,value) VALUES('history_migrated_v3','1')")
...
def get_application(user_id: int) -> Optional[sqlite3.Row]:
    with db_connect() as conn:
        return conn.execute(
            "SELECT * FROM application_history WHERE user_id=? ORDER BY id DESC LIMIT 1",
            (user_id,),
        ).fetchone()
...
def get_today_application(user_id: int) -> Optional[sqlite3.Row]:
    with db_connect() as conn:
        return conn.execute(
            "SELECT * FROM application_history WHERE user_id=? AND event_date=? ORDER BY id DESC LIMIT 1",
            (user_id, today_kst()),
        ).fetchone()
...
def save_application(user_id: int, name: str, username: str, media_type: str, media_count: int = 1) -> int:
    with db_connect() as conn:
        cur = conn.execute("""
            INSERT INTO application_history
            (user_id,name,username,status,event_date,created_at,media_type,media_count,admin_notified)
            VALUES (?,?,?,'pending',?,?,?,?,0)
        """, (user_id,name,username,today_kst(),now_kst(),media_type,media_count))
        return cur.lastrowid


def mark_admin_notified(application_id: int, value: bool) -> None:
    with db_connect() as conn:
        conn.execute("UPDATE application_history SET admin_notified=? WHERE id=?", (1 if value else 0, application_id))


def update_status(application_id: int, status: str, processed_by: Optional[int] = None) -> None:
    with db_connect() as conn:
        conn.execute("UPDATE application_history SET status=?, processed_at=?, processed_by=? WHERE id=?", (status, now_kst(), processed_by, application_id))


def delete_application(application_id: int) -> None:
    with db_connect() as conn:
        conn.execute("DELETE FROM application_history WHERE id=?", (application_id,))
...
def admin_keyboard(application_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("✅ 승인", callback_data=f"approve:{application_id}"),
            InlineKeyboardButton("❌ 거절", callback_data=f"reject:{application_id}"),
        ],
        [
            InlineKeyboardButton("🚫 차단", callback_data=f"block:{application_id}"),
            InlineKeyboardButton("🗑 신청삭제", callback_data=f"delete:{application_id}"),
        ],
    ])
...
def admin_home_keyboard() -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton("📅 날짜별 참여내역", callback_data="admin:history"),
            InlineKeyboardButton("🆔 ID별 참여이력", callback_data="admin:user_history"),
        ],
        [
...
async def send_status(message, user_id: int) -> None:
    row = get_today_application(user_id)
    if not row:
...
async def send_stats(message) -> None:
    ...
    with db_connect() as conn:
        ...
        total_all = conn.execute("SELECT COUNT(*) AS cnt FROM application_history").fetchone()["cnt"]
...
def make_admin_caption(user, application_id: int) -> str:
    ...
    return f"📌 <b>신청번호</b> : <code>#{application_id}</code>\n{base}"
...
async def handle_single_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not await check_user_before_submit(message, user):
        return
    application_id = save_application(user.id, user.full_name, f"@{user.username}" if user.username else "없음", "photo", 1)
    try:
        await context.bot.send_photo(
            chat_id=ADMIN_ID,
            photo=message.photo[-1].file_id,
            caption=make_admin_caption(user, application_id),
            parse_mode=ParseMode.HTML,
            reply_markup=admin_keyboard(application_id),
        )
        mark_admin_notified(application_id, True)
    except Exception:
        logger.exception("관리자 사진 알림 전송 실패 application_id=%s admin_id=%s", application_id, ADMIN_ID)
        update_status(application_id, "notify_failed")
        await message.reply_text("⚠️ 담당자에게 인증자료를 전달하지 못했습니다. 잠시 후 다시 제출해주세요.")
        return
    await message.reply_text(render_setting("received_text"), parse_mode=ParseMode.HTML)
...
async def process_album_group(context: ContextTypes.DEFAULT_TYPE, media_group_id: str) -> None:
    ...
    if not data:
        return
    user, chat_id, photos = data["user"], data["chat_id"], data["photos"][:10]
    row = get_today_application(user.id)
    if row and row["status"] in {"pending", "approved", "blocked"}:
        return
    application_id = save_application(user.id, user.full_name, f"@{user.username}" if user.username else "없음", "album", len(photos))
    media=[]
    caption=make_admin_caption(user, application_id)
    for i,file_id in enumerate(photos):
        media.append(InputMediaPhoto(media=file_id, caption=caption if i==0 else None, parse_mode=ParseMode.HTML if i==0 else None))
    try:
        await context.bot.send_media_group(chat_id=ADMIN_ID, media=media)
        await context.bot.send_message(
            chat_id=ADMIN_ID,
            text=f"📎 <b>사진 묶음 신청</b>\n\n📌 신청번호 : <code>#{application_id}</code>\n사진 수 : <b>{len(photos)}장</b>\n고유 ID : <code>{user.id}</code>",
            parse_mode=ParseMode.HTML,
            reply_markup=admin_keyboard(application_id),
        )
        mark_admin_notified(application_id, True)
    except Exception:
        logger.exception("관리자 앨범 알림 전송 실패 application_id=%s admin_id=%s", application_id, ADMIN_ID)
        update_status(application_id, "notify_failed")
        await context.bot.send_message(chat_id=chat_id, text="⚠️ 담당자에게 인증자료를 전달하지 못했습니다. 잠시 후 다시 제출해주세요.")
        return
    await context.bot.send_message(chat_id=chat_id, text=render_setting("received_album_text", count=len(photos)), parse_mode=ParseMode.HTML)
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_admin(user.id) and context.user_data.get("search_mode"):
        ...
        return
    if is_admin(user.id) and context.user_data.pop("history_date_mode", False):
        await send_date_history(message, message.text.strip())
        return
    if is_admin(user.id) and context.user_data.pop("user_history_mode", False):
        value=message.text.strip()
        if value.isdigit():
            await send_user_history(message, int(value))
        else:
            await message.reply_text("숫자 ID만 입력해주세요.")
        return
...
async def send_date_history(message, date: str) -> None:
    with db_connect() as conn:
        rows = conn.execute("SELECT * FROM application_history WHERE event_date=? ORDER BY created_at DESC LIMIT 100", (date,)).fetchall()
    if not rows:
        await message.reply_text(f"📭 {date} 참여 내역이 없습니다.", reply_markup=admin_home_keyboard())
        return
    names={"pending":"⏳ 대기","approved":"✅ 승인","rejected":"❌ 거절","blocked":"🚫 차단","notify_failed":"⚠️ 전달 실패"}
    lines=[f"📅 <b>{escape(date)} 참여 내역</b>\n"]
    for r in rows:
        lines.append(f"📌 신청 #{r['id']}\n👤 {escape(r['name'] or '이름 없음')}\n🆔 <code>{r['user_id']}</code>\n📊 {names.get(r['status'], r['status'])}\n🕒 {r['created_at']}\n──────────────")
    await message.reply_text("\n".join(lines)[:4000], parse_mode=ParseMode.HTML, reply_markup=admin_home_keyboard())


async def send_user_history(message, user_id: int) -> None:
    with db_connect() as conn:
        rows = conn.execute("SELECT * FROM application_history WHERE user_id=? ORDER BY id DESC LIMIT 100", (user_id,)).fetchall()
    if not rows:
        await message.reply_text("참여 이력이 없습니다.", reply_markup=admin_home_keyboard())
        return
    names={"pending":"⏳ 대기","approved":"✅ 승인","rejected":"❌ 거절","blocked":"🚫 차단","notify_failed":"⚠️ 전달 실패"}
    lines=[f"🆔 <b>{user_id} 참여 이력</b>\n"]
    for r in rows:
        lines.append(f"📅 {r['event_date']} / 신청 #{r['id']}\n📊 {names.get(r['status'], r['status'])}\n🕒 {r['created_at']}\n──────────────")
    await message.reply_text("\n".join(lines)[:4000], parse_mode=ParseMode.HTML, reply_markup=admin_home_keyboard())
...
async def user_history_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not is_admin(update.effective_user.id):
        return
    if not context.args or not context.args[0].isdigit():
        await update.effective_message.reply_text("사용법: /userhistory 숫자ID")
        return
    await send_user_history(update.effective_message, int(context.args[0]))
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "admin:history":
        ...
        await query.edit_message_text("확인할 날짜를 YYYY-MM-DD 형식으로 입력해주세요.")
        return

    if data == "admin:user_history":
        context.user_data["user_history_mode"] = True
        await query.edit_message_text("확인할 회원의 텔레그램 숫자 ID를 입력해주세요.")
        return
    ...
    if row and row["status"] in {"approved", "rejected", "blocked"}:
        await query.answer("이미 처리된 신청입니다.", show_alert=True)
        return
    if action == "approve":
        update_status(application_id, "approved", query.from_user.id)
        await safe_send_user(context, user_id, render_setting("approved_text"))
        result = "✅ 승인 완료"
    elif action == "reject":
        update_status(application_id, "rejected", query.from_user.id)
        await safe_send_user(context, user_id, render_setting("rejected_text"))
        result = "❌ 거절 완료"
    elif action == "block":
        update_status(application_id, "blocked", query.from_user.id)
        ...
        return
    result_text = f"\n\n━━━━━━━━━━━━━━\n<b>{result}</b>\n📌 신청번호 <code>#{application_id}</code>\n🆔 회원 ID <code>{user_id}</code>\n👮 처리자 <code>{query.from_user.id}</code>\n🕒 {now_kst()}\n━━━━━━━━━━━━━━"
    try:
        if query.message.caption is not None:
            await query.edit_message_caption(caption=f"{query.message.caption}{result_text}", parse_mode=ParseMode.HTML, reply_markup=None)
        elif query.message.text is not None:
            await query.edit_message_text(text=f"{query.message.text}{result_text}", parse_mode=ParseMode.HTML, reply_markup=None)
        else:
...
def main() -> None:
    ...
    logger.info("신사 이벤트 참여봇 V3 실행 중")
